...
    // ** REVISED INSTRUCTION for Interactive Plan JSON **
    "plan": `You are a world-class life coach named RyGuy. Your goal is to create a detailed, 10 to 12 step plan for the user's goal. For each step, provide a short, actionable title and a helpful, detailed description (the supplemental information) to aid the user. Return ONLY a single, valid JSON object following this structure: {"goalTitle": "A concise title based on the user's goal", "planSteps": [{"step": 1, "title": "Step 1 Title", "detail": "Supplemental information for step 1."}, ...]}. Do not include any extra text, code fences (like \`\`\`json\`), or markdown outside the JSON object.`,
    // ** END REVISED INSTRUCTION **
...
    // If feature requires JSON output, enforce it
    if (feature === "smart_goal_structuring" || feature === "plan") {
        // If 'plan', use the updated JSON instruction
        systemInstructionText = SYSTEM_INSTRUCTIONS[feature];
    }
...
    // ** MODIFIED LOGIC: Handle Plan JSON and SMART Goal JSON **
    if (feature === "smart_goal_structuring" || feature === "plan") {
        try {
            // Check if the response is already clean JSON (ideal)
            let jsonOutput;
            try {
                jsonOutput = JSON.parse(rawText.replace(/```json\s*|```/g, '').trim());
            } catch (e) {
...
                statusCode: 200,
                headers: CORS_HEADERS,
                // Return the structured JSON inside the correct key for the frontend
                body: JSON.stringify({ [feature]: jsonOutput }) // Use 'plan' or 'smart_goal_structuring' as the key
            };
        } catch (jsonError) {
            console.error(`Failed to parse ${feature.toUpperCase()} JSON:`, jsonError, rawText);
            // Fallback: send as a wrapper object containing the raw text for front-end error handling
            return {
                statusCode: 200,
                headers: CORS_HEADERS,
                body: JSON.stringify({ [feature]: { error: "Failed to parse JSON", rawText } })
            };
        }
    }
    // ** END MODIFIED LOGIC **
