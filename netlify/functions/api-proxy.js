...
async function fetchWithRetry(url, options, maxRetries = 3) {
    ...
    // Note: This line should be unreachable if logic above is correct, but safe to keep for clarity
...
        // 1. Check for the pre-parsed 'data' field (Ideal case for responseMimeType: application/json)
        if (candidate?.data) {
            structuredData = candidate.data;
        } 
        // 2. Fallback: Check the 'text' field (when it returns a JSON string)
        else if (candidate?.text) {
            const rawText = candidate.text.trim();
            try {
                // If the model wrapped the JSON in markdown fences (e.g., ```json{...}```), strip them.
