// --- Global Constants (Must be defined via environment variables) ---
const CORS_HEADERS = {
...
// Define which features require a paid membership and Firestore access
const DATA_OPERATIONS = ['SAVE_DREAM', 'LOAD_DREAMS', 'DELETE_DREAM'];

// Define which features use the standard text generation model
...
/**
 * Robust fetch wrapper that attempts a request multiple times on transient errors.
 ...
 * Currently only supports basic types (string, number, boolean, null, array, object).
 * For simplicity, we convert all numbers to integerValue if they have no decimal, or doubleValue otherwise.
 ...
 * @param {object} firestoreValue - The Firestore value object.
 * @returns {any} The decoded JS value.
 */
...
exports.handler = async function(event) {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY.trim() === '') {
        return {
            statusCode: 500,
            headers: CORS_HEADERS,
            body: JSON.stringify({ message: 'AI API Key (GEMINI_API_KEY) is not configured.' })
        };
    }
    ...
    try {
        ...
        if (feature === 'tts') {
            ...
            // FIX: Use fetchWithRetry AND correct the URL to TTS_API_URL
        ...
        if (TEXT_GENERATION_FEATURES.includes(feature)) {
            ...
            if (feature === 'smart_goal_structuring') {
                ...
                // 1. Check for the pre-parsed 'data' field (Ideal case)
                if (candidate?.data) {
                    structuredData = candidate.data;
                } 
                // 2. Fallback: Check the 'text' field and manually parse (if JSON is returned as a string)
