...
        const parsedContent = JSON.parse(cleanedText); 
    ...
    // 5a. Call Imagen API using the prompt generated by Gemini
        const imagePromptForImagen = parsedContent.image_prompt;
        
        // Ensure image prompt exists before calling (good practice)
        if (!imagePromptForImagen) {
             console.error("Prime Directive failed to generate image prompt.");
             // Continue without image, but log error
             const imagenData = { imageUrl: null, altText: null };
        } else {
             // Call the helper function you defined in Step 1
             var imagenData = await generateImagenData(imagePromptForImagen); 
        }
        
        // 5b. The final return is now modified to include image data
        return {
            statusCode: 200,
            headers: CORS_HEADERS,
            body: JSON.stringify({
                ...
                imageUrl: imagenData.imageUrl, // 💡 ADDED
                altText: imagenData.altText // 💡 ADDED
            })
        };
