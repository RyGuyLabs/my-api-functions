...
// ------------------------------------------------------------------
// NEW FEATURE: SMART GOAL CARDS (text + images for each S/M/A/R/T category)
// ------------------------------------------------------------------
if (feature === "smart_goal_cards") {
    if (!userGoal) {
        return {
            statusCode: 400,
            headers: CORS_HEADERS,
            body: JSON.stringify({ message: "Missing required userGoal." })
        };
    }
    ...
    // 1️⃣ Generate SMART Goal structured JSON
    const smartPayload = {
        contents: [{ parts: [{ text: userGoal }] }],
        systemInstruction: { parts: [{ text: SYSTEM_INSTRUCTIONS.smart_goal_structuring }] }
    };

    const smartResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${TEXT_MODEL}:generateContent?key=${GEMINI_API_KEY}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(smartPayload)
    });

    if (!smartResponse.ok) throw new Error("SMART Goal text generation failed");
    const smartResult = await smartResponse.json();
    const rawText = smartResult.candidates?.[0]?.content?.parts?.[0]?.text;
    let smartJson;
    try {
        smartJson = JSON.parse(rawText); // { Specific: {...}, Measurable: {...}, ... }
    } catch {
        return {
            statusCode: 500,
            headers: CORS_HEADERS,
            body: JSON.stringify({ message: "Failed to parse SMART Goal JSON.", rawText })
        };
    }

    // 2️⃣ Generate image for each category
    const cardPromises = Object.keys(smartJson).map(async (category) => {
        const prompt = `Visualize the goal category "${category}" for achieving: ${userGoal}`;
        const imagenPayload = {
            instances: [{ prompt }],
            parameters: { sampleCount: 1, aspectRatio: "1:1", outputMimeType: "image/png" }
        };

        const imageResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${IMAGE_MODEL}:predict?key=${GEMINI_API_KEY}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(imagenPayload)
        });
        ...
        const base64 = imageData?.predictions?.[0]?.bytesBase64Encoded || null;

        return {
            title: category,
            text: smartJson[category].description,
