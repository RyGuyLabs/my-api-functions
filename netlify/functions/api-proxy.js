...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        const { feature, userGoal, audio } = body; // Destructure audio as well
        ...
        let response = null;
        let responseIsJson = false; // Flag to handle JSON vs. text responses
        ...
        if (response && response.response) {
            ...
            if (responseIsJson) {
                try {
                    parsedBody = JSON.parse(responseText.replace(/```json/g, '').replace(/```/g, '').trim());
                } catch (jsonError) {
                    parsedBody = { text: responseText };
                }
            } else {
                parsedBody = { text: responseText };
            }
