...
export class CordataStore {
  ...
  static toSqlStatements(leadPayload) {
    ...
    const ledgerSql = `
      INSERT INTO cordata_raw_ledger (document_number, raw_record_1440)
      VALUES ($1, $2);
    `;
    const ledgerParams = [payload.ledgerRecord.documentNumber, payload.ledgerRecord.rawRecord];

    const entitySql = `
      INSERT INTO cordata_entities (
        document_number, legal_name, status, entity_type, filing_date, effective_date, fei_number, state_of_inc, principal_address, mailing_address
    ...
    return {
      ledger: { sql: ledgerSql, params: ledgerParams },
      entity: { sql: entitySql, params: entityParams },
