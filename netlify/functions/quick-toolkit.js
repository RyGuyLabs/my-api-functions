...
    'Access-Control-Allow-Origin': '*' // This line is the fix.
  };
...
  switch (tool) {
    case 'tip':
      prompt = 'Provide one short, actionable tip on a specific communication skill, a sales technique, or a public speaking strategy. Avoid general advice like "practice."';
      break;
    case 'affirmation':
...
                statusCode: 400,
                headers,
                body: JSON.stringify({ error: "Missing 'customer' or 'service' query parameters for ICP." })
            };
        }
...
                statusCode: 400,
                headers,
                body: JSON.stringify({ error: "Missing 'customer' or 'service' query parameters for pitch." })
            };
        }
