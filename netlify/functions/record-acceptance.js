// backend.js
const fs = require("fs");
const path = "./acceptLogs.json"; // persistent storage file
const SECRET = process.env.RG_TERMS_SECRET; // server-only
...
exports.handler = async (event) => {
  ...
  let logs = [];
  try { logs = JSON.parse(fs.readFileSync(path, "utf8") || "[]"); } catch {}
