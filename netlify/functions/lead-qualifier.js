...
/**
 * Extracts a JSON object from a string that might be wrapped in code fences or have extra text.
 * @param {string} text - The raw string from the AI response.
...
    // Match the first JSON object enclosed in {}
    const match = cleaned.match(/{[\s\S]*}/);
    if (match) {
      return JSON.parse(match[0]);
    }

    // If no match, try to parse the entire cleaned string
    return JSON.parse(cleaned);

  } catch (err) {
    // Re-throw with a more generic message for upstream error handling
...
          }
        });
        // Use SDK structured response if available
        if (response?.candidates?.[0]?.content?.parts?.[0]?.text) {
          return response.candidates[0].content.parts[0].text;
        }
        // Fallback to the more direct response.text()
...
      // Fill missing fields with defaults if schema fails
      if (!validate(parsedData)) {
        parsedData = { ...FALLBACK_RESPONSE, ...parsedData };
      }
    } catch (err) {
      console.error("Error within AI call block:", err);
      let message = err.message || "Unknown error occurred.";
      if (err instanceof SyntaxError || err.message === "JSON parsing failed") {
        message = "JSON parsing failed.";
      }
      parsedData = fallbackResponse(message, rawAIResponse, validate.errors);
    }
